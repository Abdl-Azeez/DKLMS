    ...
    <navigation  class='fixed-top navbar navbar-expand-lg navbar-dark nav-style'>
      ...
      <div class='collapse navbar-collapse' id='navbarNav'>
        <ul class='ul-class nav navbar-nav borderEffect'>
         ...
         <li class='activated'><a href='\DK\HR\AllRecords.php' style='color: #121307;'>Records</a></li>
    ...
    <div class='component'>
      ...
      <div style="display: flex;justify-content: space-around;width: 100%;margin-top: 50px;">
        ...
        <?php
          ...
          $user=$_SESSION['username'];
          $status=0;
          $sql= "SELECT request.Id, request.StaffName, request.StaffID, request.leaveType, request.DateFrom, request.DateTo, request.NoDays, request.PostingDate, request.Position, request.status,
           users.ELtaken, users.ELbalance, users.CLtaken, users.CLbalance, users.Annualtaken, users.Annualbalance, users.OLtaken, users.OLbalance FROM request INNER JOIN users ON users.Department = request.Departments WHERE users.StaffID = request.StaffID
           AND request.status>$status AND request.status!=3 AND request.status<>2 ORDER BY  request.AdminDate desc";
          $result= mysqli_query ($connection, $sql) or die ("Could not execute query");
              ...
              echo"
                      <tbody>
                        <tr>
                          ...
                          <td id=\"datefromto\"'>$myrow[5]</td>
                          <td id=\"NoDays\"'>$myrow[6]</td>
                          <td id=\"tdstatus\">";
                              if($myrow[9]==1){
