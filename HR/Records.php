<?php
  include('session.php');
?>
  <title>L_M_S | HR |RECORDS PAGE</title>
          ...
          <ul id="dropdown1" class="dropdown-content" style="width: 46px; position: absolute;top: 0px;left: 1034.75px;opacity: 1; display: none;">
            <li class="notificatoins-dropdown-container">
              <ul>
                ...
                <?php
                $user=$_SESSION['username'];
                $sql2 = "SELECT request.Id AS lid, request.StaffName, request.StaffID, request.leaveType, request.DateFrom, request.DateTo, request.Attachment, request.PostingDate, request.status, request.Position FROM request
      ...
      <div style="display: flex;justify-content: space-around;width: 100%;margin-top: 50px;">
        ...
        <?php
          ...
          $sql= "SELECT request.Id, request.StaffName, request.StaffID, request.leaveType, request.DateFrom, request.DateTo, request.NoDays, request.PostingDate, request.Position, request.status,
           users.ELtaken, users.ELbalance, users.CLtaken, users.CLbalance, users.Annualtaken, users.Annualbalance, users.OLtaken, users.OLbalance FROM request INNER JOIN users ON users.StaffID = request.StaffID WHERE users.StaffID = request.StaffID
           AND request.status=$status  ORDER BY request.ActionDate desc";
          $result= mysqli_query ($connection, $sql) or die ("Could not execute query");
