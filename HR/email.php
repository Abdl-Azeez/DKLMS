...
$message   = <<<EOT
    <html>
       <body>
          <h>$leave you applied for as been $status by the Human Resource, kindly log in to the system to view the request details</h>
       </body>
    </html>
EOT;
$mail = new PHPMailer();
...
$mail->Subject = "LEAVE ACKNOWLEDGEMENT BY HR"; // Mail subject
$mail->Body    = $message;
$mail->AddAddress($myrow[1]);
if (!$mail->Send()) {
    echo "Action recorded but fail to send to Staff mail";
