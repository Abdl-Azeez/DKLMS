  ...
  if (isset($_POST['StaffName']))
    {
      ...
      else
      {
        ...
        if(empty($errors)==true){
           move_uploaded_file($file_tmp,"/DK/Staff/uploadeddata/".$file_name);

        } else{
           print_r($errors);
        }
        if ($errors ==false){
          if (($leaveType==1) && ($myrow[16]!=0) && ($myrow[16]>=$NoDays))
          {
            $sql="INSERT INTO request (StaffName, StaffID, Position, departments, leaveType, DateFrom, DateTo, Reason, Attachment, NoDays) VALUES ('$StaffName', '$StaffID', '$Position', '$departments', '$leaveType', '$DateFrom', '$DateTo', '$Reason', '$file_name', '$NoDays')";
            $result=mysqli_query($connection, $sql) or die ("Couldn't execute query");

            if ($result)
            {
              $sql1="SELECT * FROM users WHERE roles='Admin'";
          ...
          else if (($leaveType==1) && ($myrow[16]==0)){
            echo "You have exceeded all your Annual Leaves";
          }
          else if (($leaveType==1) && ($myrow[16]>0) && ($myrow[16]<$NoDays)){
            echo "Your leave balance is not up to the days you have applied for\nKindly reduce the number of days you're applying for";
          }

          else if (($leaveType==2) && ($myrow[16]!=0) && ($myrow[16]>=$NoDays))
          {
            $sql="INSERT INTO request (StaffName, StaffID, Position, departments, leaveType, DateFrom, DateTo, Reason, Attachment, NoDays) VALUES ('$StaffName', '$StaffID', '$Position', '$departments', '$leaveType', '$DateFrom', '$DateTo', '$Reason', '$file_name', '$NoDays')";
            $result=mysqli_query($connection, $sql) or die ("Couldn't execute query");
          ...
          else if (($leaveType==2) && ($myrow[16]==0))
          {
            echo "You have exceeded all your Annual Leaves\nYou're not eligible to apply for Emergency Leave";
          }
          else if (($leaveType==2) && ($myrow[16]>0) && ($myrow[16]<$NoDays)){
            echo "Your leave balance is not up to the days you have applied for\nKindly reduce the number of days you're applying for";
          }

          else if (($leaveType==3) && ($myrow[13]!=0) && ($myrow[13]>=$NoDays))
          {
            $sql="INSERT INTO request (StaffName, StaffID, Position, departments, leaveType, DateFrom, DateTo, Reason, Attachment, NoDays) VALUES ('$StaffName', '$StaffID', '$Position', '$departments', '$leaveType', '$DateFrom', '$DateTo', '$Reason', '$file_name', '$NoDays')";
            $result=mysqli_query($connection, $sql) or die ("Couldn't execute query");
          ...
          else if (($leaveType==3) && ($myrow[13]==0))
          {
          ...
          else if (($leaveType==4) && ($myrow[19]!=0) && ($myrow[19]>=$NoDays))
          {
            $sql="INSERT INTO request (StaffName, StaffID, Position, departments, leaveType, DateFrom, DateTo, Reason, Attachment, NoDays) VALUES ('$StaffName', '$StaffID', '$Position', '$departments', '$leaveType', '$DateFrom', '$DateTo', '$Reason', '$file_name', '$NoDays')";
            $result=mysqli_query($connection, $sql) or die ("Couldn't execute query");
